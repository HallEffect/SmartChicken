...
def foo():
    while True:
        ...
        humidity, temp = getCurrentTempHum()
...
def index():
    ...
    templateData['main_lamp'] = MAIN_LAMP.is_active
    templateData['heat_lamp'] = HEAT_LAMP.is_active
    templateData['radiator'] = RADIATOR.is_active

    if my_proc.is_alive():
        templateData['auto'] = True
...
def action(changePin, action):
    ...
    templateData['auto']= False

    if changePin == 'main_lamp':
        if action == 'off':
            MAIN_LAMP.off()
        else:
            MAIN_LAMP.on()
            templateData['main_lamp'] = True
    else:
        templateData['main_lamp'] = MAIN_LAMP.is_active

    if changePin == 'heat_lamp':
        if action == 'off':
            HEAT_LAMP.off()
        else:
            HEAT_LAMP.on()
            templateData['heat_lamp'] = True
    else:
        templateData['heat_lamp'] = HEAT_LAMP.is_active

    if changePin == 'radiator':
        if action == 'off':
            RADIATOR.off()
        else:
            RADIATOR.on()
            templateData['radiator'] = True
    else:
        templateData['radiator'] = RADIATOR.is_active
